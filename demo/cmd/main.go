...
func main() {
	...
	case 1, 3:
		...
		StartGin()
	case 2:
		GrpcIndexerMain() // 2:以 grpc server 的方式启动索引服务
	}
}

// go run ./demo/cmd -mode=1 -index=true -port=5678 -dbPath=temp/local_db/video_bolt
// go run ./demo/cmd -mode=2 -index=true -port=5600 -dbPath=temp/local_db/video_bolt -totalWorkers=2 -workerIndex=0
// go run ./demo/cmd -mode=2 -index=true -port=5601 -dbPath=temp/local_db/video_bolt -totalWorkers=2 -workerIndex=1
// go run ./demo/cmd -mode=3 -index=true -port=5678
